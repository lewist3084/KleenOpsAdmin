...
class TeamFileImages {
  static Future<List<Map<String, dynamic>>> headerImageEntries({
    ...
    try {
      final snap = await FirebaseFirestore.instance
          .collection('file')
          .where('teamRef', isEqualTo: teamRef)
          .where('teamMediaRole', isEqualTo: 'header')
          .get();
  ...
  static Future<void> syncHeaderImages({
    ...
    for (var i = 0; i < normalized.length; i += 1) {
      ...
      final teamName =
          name != null && name.trim().isNotEmpty ? name.trim() : 'Team';
      ...
      final payload = <String, dynamic>{
        ...
        'downloadUrl': url,
        'fileUrl': url,
        'name': '$teamName Image ${i + 1}',
        'mediaType': 'Image',
        'fileType': 'image',
        'teamRef': teamRef,
        'teamMediaRole': 'header',
        'order': order,
        'isMaster': isMaster,
