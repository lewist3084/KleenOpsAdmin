...
class ObjectProcessFileImages {
  static Future<List<Map<String, dynamic>>> headerImageEntries({
    required DocumentReference<Map<String, dynamic>> companyRef,
    required DocumentReference processRef,
  }) async {
    try {
      final snap = await companyRef
          .collection('file')
          .where('objectProcessRef', isEqualTo: processRef)
          .where('objectProcessMediaRole', isEqualTo: 'header')
          .get();
  ...
  static Future<void> syncHeaderImages({
    ...
    for (var i = 0; i < normalized.length; i += 1) {
      ...
      final displayName = processName != null && processName.trim().isNotEmpty
          ? processName.trim()
          : 'Process';
      ...
      final payload = <String, dynamic>{
        ...
        'mediaType': 'Image',
        'fileType': 'image',
        'objectProcessRef': processRef,
        'objectProcessId': processRef.id,
        'objectProcessMediaRole': 'header',
        'order': order,
        'isMaster': isMaster,
