...
/// Streaming STT client that pushes raw PCM frames over WebSocket to a
/// server-side Google Speech streaming proxy and emits interim/final text.
class GoogleStreamingTranscriber implements LiveChunkedTranscriber {
  ...
  /// Use this to persist speaker-attributed utterances during a call.
  ...
  Future<void> pause() async {
    // For streaming, pause behaves like stop: close audio stream but keep text.
    if (!_recording) return;
  ...
  Future<RecordingArtifacts> buildRecordingArtifacts() async {
    // TODO: Optionally buffer and emit audio artifacts if needed.
      ...
      RecordConfig(
        ...
        androidConfig: const AndroidRecordConfig(
          service: AndroidService(
            title: 'Meeting Minutes (Streaming)',
            content: 'Recording in the background',
          ),
        ),
  ...
  Future<void> _openChannel() async {
    try {
      _channel = IOWebSocketChannel.connect(_endpoint.toString());
    } catch (err) {
      _channel = null;
  ...
  void _startKeepAlive() {
    _stopKeepAlive();
    // Keep a steady trickle of silence when no audio arrives to avoid STT timeouts.
    // Send ~100ms of silence so the stream keeps real-time pace (16kHz mono PCM16 -> 3200 bytes).
