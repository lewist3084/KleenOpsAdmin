...
/// Displays the Profit & Loss sections for the current company.
///
/// Sections come from the `companyProfitLossSection` subcollection under the
...
class FinanceProfitLossContent extends ConsumerStatefulWidget {
  const FinanceProfitLossContent({super.key});
...
class _FinanceProfitLossContentState
  ...
  Future<double> _calcSectionTotal({
    ...
    final accountSnap = await FirebaseFirestore.instance
        .collection('account')
        .where('profitLossId', isEqualTo: sectionRef)
        .get();
    ...
    return totals.fold<double>(0.0, (acc, val) => acc + val);
  }
          ...
          builder: (context, sectionSnap) {
            ...
            if (sectionSnap.connectionState == ConnectionState.waiting) {
              ...
              body = StandardViewGroup(
                ...
                groupBy: (doc) =>
                    doc.data()['profitLossId'] as DocumentReference?,
                itemSort: (a, b) {
