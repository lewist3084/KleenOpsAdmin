...
import '../details/safety_analysis_process_failure_mode_potential_cause_details.dart';
import 'package:kleenops_admin/common/utils/snackbar_service.dart';

class PotentialCauseTabsScreen extends StatelessWidget {
  final DocumentReference<Map<String, dynamic>> failureModeRef;

  const PotentialCauseTabsScreen({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: AiScreenContext(
        ...
        child: _wrapCanvas(
          PotentialCauseTabs(failureModeRef: failureModeRef),
        ),
      ),
    );
  }
...
class PotentialCauseTabs extends StatefulWidget {
  final DocumentReference<Map<String, dynamic>> failureModeRef;

  const PotentialCauseTabs({
    super.key,
    required this.failureModeRef,
  });

  @override
  State<PotentialCauseTabs> createState() => _PotentialCauseTabsState();
}

class _PotentialCauseTabsState extends State<PotentialCauseTabs>
  ...
  DocumentReference<Map<String, dynamic>> get _companyRef =>
      widget.failureModeRef.parent.parent!;
  ...
  Widget _buildPotentialCauseList(
    ...
    final stream = _companyRef
        ...
        .where('failureModeId', isEqualTo: widget.failureModeRef)
        .snapshots();
        ...
        builder: (ctx2, setState) => DialogAction(
          ...
          onAction: () async {
            ...
            await FirestoreService().saveDocument(
              ...
              data: {
                'failureModeId': widget.failureModeRef,
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      stream: _failureModeStream ??= widget.failureModeRef.snapshots(),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return const Center(child: CircularProgressIndicator());
        }
        if (snapshot.hasError || !snapshot.hasData || !snapshot.data!.exists) {
          return const Center(child: Text('Failure mode not found.'));
        }
          ...
          builder: (ctx, snap) {
            ...
            final effectName =
                (snap.data![1]?.data() as Map<String, dynamic>?)?['name'] ??
                    'Unknown';
            final severityData = snap.data![2]?.data() as Map<String, dynamic>?;
            final severityName =
                              ...
                              ListView(
                                ...
                                children: [
                                  ContainerActionWidget(
                                    ...
                                    actionText: 'Add',
                                    onAction: () =>
                                        _showAddPotentialCauseDialog(context),
